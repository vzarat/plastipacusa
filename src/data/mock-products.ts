import { ProductWithVariants } from "@/types";

// Active industrial stretch film dataset strictly limited to FORCE Standard series
export const FALLBACK_PRODUCTS: ProductWithVariants[] = [
  // 1. FORCE STANDARD (50 Gauge Hand Stretch Film)
  {
    id: 1,
    slug: "stretch-film-18-x-50-ga-x-1000ft",
    title: 'STRETCH FILM 18" X 50 GA X 1000FT',
    name: 'STRETCH FILM 18" X 50 GA X 1000FT',
    brand: "FORCE",
    application: "hand",
    categorySlug: "force-standard",
    filmType: "Cast Co-Extruded Multi-Layer",
    color: "Ultra Clear",
    shortDescription:
      "Premium industrial cast hand wrap engineered for high load retention, exceptional puncture resistance, and quiet unwind.",
    description:
      "Plastipac FORCE™ Standard Hand Stretch Film is manufactured with cutting-edge multi-layer cast extrusion technology. Engineered specifically for manual pallet wrapping operations, it provides superior cling, maximum puncture resistance on sharp corners, and whisper-quiet release to protect operators from fatigue.",
    features: [
      "High optical clarity for barcode scanning through wrap",
      "One-side cling prevents pallets from sticking together during transit",
      "Differential slip release for effortless dispensing",
      "Ultra-strong puncture & tear resistance for irregular loads",
      "High load retention & maximum stretch recovery for stable pallet freight",
    ],
    techSheetUrl: "/docs/plastipac-force-hand-film-specs.pdf",
    imageUrl:
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/productos_plastipac_manual.png",
    images: [
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/productos_plastipac_manual.png",
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/CAJA_MANUAL_PLASTIPAC.png",
    ],
    recommendedUsage:
      "Manual wrapping of B and C type pallet loads, distribution centers, cross-border freight, and warehouse logistics.",
    createdAt: new Date(),
    updatedAt: new Date(),
    variants: [
      {
        id: 1,
        productId: 1,
        sku: "PP-FRC-1850-1B",
        packageSize: "1 BOX WITH 4 ROLLS",
        widthInches: "18.00",
        gauge: 50,
        lengthFeet: 1000,
        rollsPerBox: 4,
        rollsPerPallet: 256,
        weightLbs: "12.00",
        priceUsd: "20.71",
        casePriceUsd: "20.71",
        palletPriceUsd: "1149.49",
        stockStatus: "in_stock",
        createdAt: new Date(),
      },
      {
        id: 2,
        productId: 1,
        sku: "PP-FRC-1850-16B",
        packageSize: "16 BOXES = 64 ROLLS",
        widthInches: "18.00",
        gauge: 50,
        lengthFeet: 1000,
        rollsPerBox: 64,
        rollsPerPallet: 256,
        weightLbs: "192.00",
        priceUsd: "316.70",
        casePriceUsd: "316.70",
        palletPriceUsd: "1149.49",
        stockStatus: "in_stock",
        createdAt: new Date(),
      },
      {
        id: 3,
        productId: 1,
        sku: "PP-FRC-1850-32B",
        packageSize: "32 BOXES = 128 ROLLS (HALF PALLET)",
        widthInches: "18.00",
        gauge: 50,
        lengthFeet: 1000,
        rollsPerBox: 128,
        rollsPerPallet: 256,
        weightLbs: "384.00",
        priceUsd: "604.07",
        casePriceUsd: "604.07",
        palletPriceUsd: "1149.49",
        stockStatus: "in_stock",
        createdAt: new Date(),
      },
      {
        id: 4,
        productId: 1,
        sku: "PP-FRC-1850-64B",
        packageSize: "64 BOXES = 256 ROLLS (FULL PALLET)",
        widthInches: "18.00",
        gauge: 50,
        lengthFeet: 1000,
        rollsPerBox: 256,
        rollsPerPallet: 256,
        weightLbs: "768.00",
        priceUsd: "1149.49",
        casePriceUsd: "1149.49",
        palletPriceUsd: "1149.49",
        stockStatus: "in_stock",
        createdAt: new Date(),
      },
    ],
  },

  // 2. FORCE STANDARD (60 Gauge Hand Stretch Film)
  {
    id: 2,
    slug: "stretch-film-18-x-60-ga-x-1000ft",
    title: 'STRETCH FILM 18" X 60 GA X 1000FT',
    name: 'STRETCH FILM 18" X 60 GA X 1000FT',
    brand: "FORCE",
    application: "hand",
    categorySlug: "force-standard",
    filmType: "Cast Co-Extruded Multi-Layer",
    color: "Ultra Clear",
    shortDescription:
      "Heavy-duty 60 Gauge industrial cast hand wrap engineered for heavier loads, superior puncture resistance, and secure pallet containment.",
    description:
      "Plastipac FORCE™ Standard 60 Gauge Hand Stretch Film provides elevated load holding strength for heavier pallets and irregular shaped freight. Manufactured with advanced multi-layer cast extrusion, it features superior cling, high puncture resistance on sharp crate corners, and whisper-quiet unwind.",
    features: [
      "Heavy-duty 60 Gauge cast resin formulation for heavier industrial pallet freight",
      "Ultra-strong corner puncture and tear propagation resistance",
      "Optically ultra-clear for swift optical barcode scanning",
      "Differential cling prevents pallets from rubbing together in transit",
      "Whisper-quiet release reduces operator strain in warehouse environments",
    ],
    techSheetUrl: "/docs/plastipac-force-hand-film-specs.pdf",
    imageUrl:
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/productos_plastipac_manual.png",
    images: [
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/productos_plastipac_manual.png",
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/CAJA_MANUAL_PLASTIPAC.png",
    ],
    recommendedUsage:
      "Manual wrapping of heavy industrial loads, irregular crate edges, beverage distribution, and cross-border transport.",
    createdAt: new Date(),
    updatedAt: new Date(),
    variants: [
      {
        id: 5,
        productId: 2,
        sku: "PP-FRC-1860-1B",
        packageSize: "1 BOX WITH 4 ROLLS",
        widthInches: "18.00",
        gauge: 60,
        lengthFeet: 1000,
        rollsPerBox: 4,
        rollsPerPallet: 256,
        weightLbs: "14.40",
        priceUsd: "24.85",
        casePriceUsd: "24.85",
        palletPriceUsd: "1379.38",
        stockStatus: "in_stock",
        createdAt: new Date(),
      },
      {
        id: 6,
        productId: 2,
        sku: "PP-FRC-1860-16B",
        packageSize: "16 BOXES = 64 ROLLS",
        widthInches: "18.00",
        gauge: 60,
        lengthFeet: 1000,
        rollsPerBox: 64,
        rollsPerPallet: 256,
        weightLbs: "230.40",
        priceUsd: "380.03",
        casePriceUsd: "380.03",
        palletPriceUsd: "1379.38",
        stockStatus: "in_stock",
        createdAt: new Date(),
      },
      {
        id: 7,
        productId: 2,
        sku: "PP-FRC-1860-32B",
        packageSize: "32 BOXES = 128 ROLLS (HALF PALLET)",
        widthInches: "18.00",
        gauge: 60,
        lengthFeet: 1000,
        rollsPerBox: 128,
        rollsPerPallet: 256,
        weightLbs: "460.80",
        priceUsd: "724.88",
        casePriceUsd: "724.88",
        palletPriceUsd: "1379.38",
        stockStatus: "in_stock",
        createdAt: new Date(),
      },
      {
        id: 8,
        productId: 2,
        sku: "PP-FRC-1860-64B",
        packageSize: "64 BOXES = 256 ROLLS (FULL PALLET)",
        widthInches: "18.00",
        gauge: 60,
        lengthFeet: 1000,
        rollsPerBox: 256,
        rollsPerPallet: 256,
        weightLbs: "921.60",
        priceUsd: "1379.38",
        casePriceUsd: "1379.38",
        palletPriceUsd: "1379.38",
        stockStatus: "in_stock",
        createdAt: new Date(),
      },
    ],
  },

  // 3. FORCE STANDARD (70 Gauge Hand Stretch Film)
  {
    id: 3,
    slug: "stretch-film-18-x-70-ga-x-1000ft",
    title: 'STRETCH FILM 18" X 70 GA X 1000FT',
    name: 'STRETCH FILM 18" X 70 GA X 1000FT',
    brand: "FORCE",
    application: "hand",
    categorySlug: "force-standard",
    filmType: "Cast Co-Extruded Multi-Layer",
    color: "Ultra Clear",
    shortDescription:
      "Extra heavy-duty 70 Gauge industrial cast hand wrap engineered for severe freight demands, maximum puncture resistance, and heavy pallet containment.",
    description:
      "Plastipac FORCE™ Standard 70 Gauge Hand Stretch Film is formulated for maximum load-holding security on the heaviest and most abrasive pallets. Engineered with advanced multi-layer cast extrusion technology, it delivers extreme puncture resistance, sharp-corner durability, high cling tension, and silent unwind to handle rigorous transport challenges.",
    features: [
      "Extra heavy-duty 70 Gauge cast resin formulation for demanding pallet freight",
      "Maximum puncture resistance against sharp crate corners and jagged pallet edges",
      "Optically ultra-clear for effortless barcode scanning and inventory tracking",
      "Superior cling retention keeps pallet loads bonded securely without unraveling",
      "Whisper-quiet release eliminates warehouse noise and minimizes operator fatigue",
    ],
    techSheetUrl: "/docs/plastipac-force-hand-film-specs.pdf",
    imageUrl:
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/productos_plastipac_manual.png",
    images: [
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/productos_plastipac_manual.png",
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/CAJA_MANUAL_PLASTIPAC.png",
    ],
    recommendedUsage:
      "Manual wrapping of heavy industrial machinery, bricks/building materials, metal components, and heavy-duty logistics freight.",
    createdAt: new Date(),
    updatedAt: new Date(),
    variants: [
      {
        id: 9,
        productId: 3,
        sku: "PP-FRC-1870-1B",
        packageSize: "1 BOX WITH 4 ROLLS",
        widthInches: "18.00",
        gauge: 70,
        lengthFeet: 1000,
        rollsPerBox: 4,
        rollsPerPallet: 256,
        weightLbs: "16.80",
        priceUsd: "28.99",
        casePriceUsd: "28.99",
        palletPriceUsd: "1609.28",
        stockStatus: "in_stock",
        createdAt: new Date(),
      },
      {
        id: 10,
        productId: 3,
        sku: "PP-FRC-1870-16B",
        packageSize: "16 BOXES = 64 ROLLS",
        widthInches: "18.00",
        gauge: 70,
        lengthFeet: 1000,
        rollsPerBox: 64,
        rollsPerPallet: 256,
        weightLbs: "268.80",
        priceUsd: "443.37",
        casePriceUsd: "443.37",
        palletPriceUsd: "1609.28",
        stockStatus: "in_stock",
        createdAt: new Date(),
      },
      {
        id: 11,
        productId: 3,
        sku: "PP-FRC-1870-32B",
        packageSize: "32 BOXES = 128 ROLLS (HALF PALLET)",
        widthInches: "18.00",
        gauge: 70,
        lengthFeet: 1000,
        rollsPerBox: 128,
        rollsPerPallet: 256,
        weightLbs: "537.60",
        priceUsd: "845.69",
        casePriceUsd: "845.69",
        palletPriceUsd: "1609.28",
        stockStatus: "in_stock",
        createdAt: new Date(),
      },
      {
        id: 12,
        productId: 3,
        sku: "PP-FRC-1870-64B",
        packageSize: "64 BOXES = 256 ROLLS (FULL PALLET)",
        widthInches: "18.00",
        gauge: 70,
        lengthFeet: 1000,
        rollsPerBox: 256,
        rollsPerPallet: 256,
        weightLbs: "1075.20",
        priceUsd: "1609.28",
        casePriceUsd: "1609.28",
        palletPriceUsd: "1609.28",
        stockStatus: "in_stock",
        createdAt: new Date(),
      },
    ],
  },

  // 4. FORCE STANDARD (80 Gauge Hand Stretch Film - 1000FT)
  {
    id: 4,
    slug: "stretch-film-18-x-80-ga-x-1000ft",
    title: 'STRETCH FILM 18" X 80 GA X 1000FT',
    name: 'STRETCH FILM 18" X 80 GA X 1000FT',
    brand: "FORCE",
    application: "hand",
    categorySlug: "force-standard",
    filmType: "Cast Co-Extruded Multi-Layer",
    color: "Ultra Clear",
    shortDescription:
      "Maximum-strength 80 Gauge industrial cast hand wrap engineered for extreme pallet holding capacity, severe sharp corner resistance, and uncompromising load security.",
    description:
      "Plastipac FORCE™ Standard 80 Gauge Hand Stretch Film delivers maximum containment force for the most demanding commercial and industrial freight. Manufactured with advanced multi-layer cast extrusion technology, it provides maximum resistance against tearing, extreme puncture durability across irregular and sharp-cornered loads, aggressive cling retention, and silent operator-friendly unwind.",
    features: [
      "Maximum-strength 80 Gauge cast resin formulation for extreme industrial freight loads",
      "Superior puncture durability engineered for jagged pallet edges, metal, and building materials",
      "Optically ultra-clear cast clarity for fast optical barcode and SKU scanning",
      "High-tension cling retention keeps oversized and heavy pallets bonded securely in transit",
      "Whisper-quiet release eliminates warehouse acoustic strain and operator fatigue",
    ],
    techSheetUrl: "/docs/plastipac-force-hand-film-specs.pdf",
    imageUrl:
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/productos_plastipac_manual.png",
    images: [
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/productos_plastipac_manual.png",
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/CAJA_MANUAL_PLASTIPAC.png",
    ],
    recommendedUsage:
      "Manual wrapping of extreme-weight pallets, steel drums, heavy machinery, bagged concrete/cement, and rough export freight.",
    createdAt: new Date(),
    updatedAt: new Date(),
    variants: [
      {
        id: 13,
        productId: 4,
        sku: "PP-FRC-1880-1B",
        packageSize: "1 BOX WITH 4 ROLLS",
        widthInches: "18.00",
        gauge: 80,
        lengthFeet: 1000,
        rollsPerBox: 4,
        rollsPerPallet: 256,
        weightLbs: "19.20",
        priceUsd: "33.14",
        casePriceUsd: "33.14",
        palletPriceUsd: "1839.18",
        stockStatus: "in_stock",
        createdAt: new Date(),
      },
      {
        id: 14,
        productId: 4,
        sku: "PP-FRC-1880-16B",
        packageSize: "16 BOXES = 64 ROLLS",
        widthInches: "18.00",
        gauge: 80,
        lengthFeet: 1000,
        rollsPerBox: 64,
        rollsPerPallet: 256,
        weightLbs: "307.20",
        priceUsd: "506.71",
        casePriceUsd: "506.71",
        palletPriceUsd: "1839.18",
        stockStatus: "in_stock",
        createdAt: new Date(),
      },
      {
        id: 15,
        productId: 4,
        sku: "PP-FRC-1880-32B",
        packageSize: "32 BOXES = 128 ROLLS (HALF PALLET)",
        widthInches: "18.00",
        gauge: 80,
        lengthFeet: 1000,
        rollsPerBox: 128,
        rollsPerPallet: 256,
        weightLbs: "614.40",
        priceUsd: "966.51",
        casePriceUsd: "966.51",
        palletPriceUsd: "1839.18",
        stockStatus: "in_stock",
        createdAt: new Date(),
      },
      {
        id: 16,
        productId: 4,
        sku: "PP-FRC-1880-64B",
        packageSize: "64 BOXES = 256 ROLLS (FULL PALLET)",
        widthInches: "18.00",
        gauge: 80,
        lengthFeet: 1000,
        rollsPerBox: 256,
        rollsPerPallet: 256,
        weightLbs: "1228.80",
        priceUsd: "1839.18",
        casePriceUsd: "1839.18",
        palletPriceUsd: "1839.18",
        stockStatus: "in_stock",
        createdAt: new Date(),
      },
    ],
  },

  // 5. FORCE STANDARD (50 Gauge Hand Stretch Film - 1500FT)
  {
    id: 5,
    slug: "stretch-film-18-x-50-ga-x-1500ft",
    title: 'STRETCH FILM 18" X 50 GA X 1500FT',
    name: 'STRETCH FILM 18" X 50 GA X 1500FT',
    brand: "FORCE",
    application: "hand",
    categorySlug: "force-standard",
    filmType: "Cast Co-Extruded Multi-Layer",
    color: "Ultra Clear",
    shortDescription:
      "Extended-length 1500FT 50 Gauge industrial cast hand wrap delivering high roll yield, exceptional economy, and puncture resistance.",
    description:
      "Plastipac FORCE™ Standard 50 Gauge 1500FT Hand Stretch Film provides an extended 1500-foot roll length for maximum wrapping yield per roll and fewer changeovers. Engineered with multi-layer cast extrusion technology, it offers superior tensile stability, excellent corner cling, and effortless unwind.",
    features: [
      "Extended 1500 FT roll length for 50% more pallet wraps per roll",
      "High-yield 50 Gauge cast resin formulation for maximum economy",
      "Ultra-clear transparency for instantaneous barcode scanning",
      "Superior cling retention keeps pallet loads bonded without unraveling",
      "Whisper-quiet release minimizes warehouse operator fatigue",
    ],
    techSheetUrl: "/docs/plastipac-force-hand-film-specs.pdf",
    imageUrl:
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/productos_plastipac_manual.png",
    images: [
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/productos_plastipac_manual.png",
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/CAJA_MANUAL_PLASTIPAC.png",
    ],
    recommendedUsage:
      "Manual pallet wrapping in high-volume distribution centers, warehouse shipping, and general logistics.",
    createdAt: new Date(),
    updatedAt: new Date(),
    variants: [
      {
        id: 17,
        productId: 5,
        sku: "PP-FRC-1850-15-1B",
        packageSize: "1 BOX WITH 4 ROLLS",
        widthInches: "18.00",
        gauge: 50,
        lengthFeet: 1500,
        rollsPerBox: 4,
        rollsPerPallet: 256,
        weightLbs: "18.00",
        priceUsd: "31.06",
        casePriceUsd: "31.06",
        palletPriceUsd: "1724.23",
        stockStatus: "in_stock",
        createdAt: new Date(),
      },
      {
        id: 18,
        productId: 5,
        sku: "PP-FRC-1850-15-16B",
        packageSize: "16 BOXES = 64 ROLLS",
        widthInches: "18.00",
        gauge: 50,
        lengthFeet: 1500,
        rollsPerBox: 64,
        rollsPerPallet: 256,
        weightLbs: "288.00",
        priceUsd: "475.04",
        casePriceUsd: "475.04",
        palletPriceUsd: "1724.23",
        stockStatus: "in_stock",
        createdAt: new Date(),
      },
      {
        id: 19,
        productId: 5,
        sku: "PP-FRC-1850-15-32B",
        packageSize: "32 BOXES = 128 ROLLS (HALF PALLET)",
        widthInches: "18.00",
        gauge: 50,
        lengthFeet: 1500,
        rollsPerBox: 128,
        rollsPerPallet: 256,
        weightLbs: "576.00",
        priceUsd: "906.10",
        casePriceUsd: "906.10",
        palletPriceUsd: "1724.23",
        stockStatus: "in_stock",
        createdAt: new Date(),
      },
      {
        id: 20,
        productId: 5,
        sku: "PP-FRC-1850-15-64B",
        packageSize: "64 BOXES = 256 ROLLS (FULL PALLET)",
        widthInches: "18.00",
        gauge: 50,
        lengthFeet: 1500,
        rollsPerBox: 256,
        rollsPerPallet: 256,
        weightLbs: "1152.00",
        priceUsd: "1724.23",
        casePriceUsd: "1724.23",
        palletPriceUsd: "1724.23",
        stockStatus: "in_stock",
        createdAt: new Date(),
      },
    ],
  },

  // 6. FORCE STANDARD (60 Gauge Hand Stretch Film - 1500FT)
  {
    id: 6,
    slug: "stretch-film-18-x-60-ga-x-1500ft",
    title: 'STRETCH FILM 18" X 60 GA X 1500FT',
    name: 'STRETCH FILM 18" X 60 GA X 1500FT',
    brand: "FORCE",
    application: "hand",
    categorySlug: "force-standard",
    filmType: "Cast Co-Extruded Multi-Layer",
    color: "Ultra Clear",
    shortDescription:
      "Extended-length 1500FT 60 Gauge heavy-duty industrial cast hand wrap engineered for heavier pallets and reduced roll change downtime.",
    description:
      "Plastipac FORCE™ Standard 60 Gauge 1500FT Hand Stretch Film provides elevated puncture resistance and load-holding force with 1500 feet of continuous cast film. Ideal for wrapping heavier carton stacks and irregular freight with fewer roll changes.",
    features: [
      "Extended 1500 FT roll length for increased warehouse efficiency",
      "Heavy-duty 60 Gauge cast resin formulation for secure pallet containment",
      "High corner tear resistance on sharp edges and corrugated corners",
      "Optically clear formulation for effortless optical inventory scanning",
      "Quiet unwind release reduces noise pollution in packing facilities",
    ],
    techSheetUrl: "/docs/plastipac-force-hand-film-specs.pdf",
    imageUrl:
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/productos_plastipac_manual.png",
    images: [
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/productos_plastipac_manual.png",
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/CAJA_MANUAL_PLASTIPAC.png",
    ],
    recommendedUsage:
      "Manual wrapping of heavy industrial loads, irregular crate edges, beverage distribution, and cross-border transport.",
    createdAt: new Date(),
    updatedAt: new Date(),
    variants: [
      {
        id: 21,
        productId: 6,
        sku: "PP-FRC-1860-15-1B",
        packageSize: "1 BOX WITH 4 ROLLS",
        widthInches: "18.00",
        gauge: 60,
        lengthFeet: 1500,
        rollsPerBox: 4,
        rollsPerPallet: 256,
        weightLbs: "21.60",
        priceUsd: "37.28",
        casePriceUsd: "37.28",
        palletPriceUsd: "2069.08",
        stockStatus: "in_stock",
        createdAt: new Date(),
      },
      {
        id: 22,
        productId: 6,
        sku: "PP-FRC-1860-15-16B",
        packageSize: "16 BOXES = 64 ROLLS",
        widthInches: "18.00",
        gauge: 60,
        lengthFeet: 1500,
        rollsPerBox: 64,
        rollsPerPallet: 256,
        weightLbs: "345.60",
        priceUsd: "570.05",
        casePriceUsd: "570.05",
        palletPriceUsd: "2069.08",
        stockStatus: "in_stock",
        createdAt: new Date(),
      },
      {
        id: 23,
        productId: 6,
        sku: "PP-FRC-1860-15-32B",
        packageSize: "32 BOXES = 128 ROLLS (HALF PALLET)",
        widthInches: "18.00",
        gauge: 60,
        lengthFeet: 1500,
        rollsPerBox: 128,
        rollsPerPallet: 256,
        weightLbs: "691.20",
        priceUsd: "1087.32",
        casePriceUsd: "1087.32",
        palletPriceUsd: "2069.08",
        stockStatus: "in_stock",
        createdAt: new Date(),
      },
      {
        id: 24,
        productId: 6,
        sku: "PP-FRC-1860-15-64B",
        packageSize: "64 BOXES = 256 ROLLS (FULL PALLET)",
        widthInches: "18.00",
        gauge: 60,
        lengthFeet: 1500,
        rollsPerBox: 256,
        rollsPerPallet: 256,
        weightLbs: "1382.40",
        priceUsd: "2069.08",
        casePriceUsd: "2069.08",
        palletPriceUsd: "2069.08",
        stockStatus: "in_stock",
        createdAt: new Date(),
      },
    ],
  },

  // 7. FORCE STANDARD (70 Gauge Hand Stretch Film - 1500FT)
  {
    id: 7,
    slug: "stretch-film-18-x-70-ga-x-1500ft",
    title: 'STRETCH FILM 18" X 70 GA X 1500FT',
    name: 'STRETCH FILM 18" X 70 GA X 1500FT',
    brand: "FORCE",
    application: "hand",
    categorySlug: "force-standard",
    filmType: "Cast Co-Extruded Multi-Layer",
    color: "Ultra Clear",
    shortDescription:
      "Extended-length 1500FT 70 Gauge extra heavy-duty industrial cast hand wrap built for severe freight and abrasive load containment.",
    description:
      "Plastipac FORCE™ Standard 70 Gauge 1500FT Hand Stretch Film combines extreme durability with an extended 1500-foot yield. Formulated for heavy freight demands, high tension retention, and maximum corner tear protection.",
    features: [
      "Extended 1500 FT roll length for extra packaging throughput",
      "Extra heavy-duty 70 Gauge resin formulation for rigorous freight holding",
      "Superior puncture durability against jagged pallet edges and metal containers",
      "Optically clear film ensures seamless barcode scanning",
      "Whisper-quiet release eliminates warehouse acoustic strain",
    ],
    techSheetUrl: "/docs/plastipac-force-hand-film-specs.pdf",
    imageUrl:
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/productos_plastipac_manual.png",
    images: [
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/productos_plastipac_manual.png",
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/CAJA_MANUAL_PLASTIPAC.png",
    ],
    recommendedUsage:
      "Manual wrapping of heavy industrial machinery, bricks/building materials, metal components, and heavy-duty logistics freight.",
    createdAt: new Date(),
    updatedAt: new Date(),
    variants: [
      {
        id: 25,
        productId: 7,
        sku: "PP-FRC-1870-15-1B",
        packageSize: "1 BOX WITH 4 ROLLS",
        widthInches: "18.00",
        gauge: 70,
        lengthFeet: 1500,
        rollsPerBox: 4,
        rollsPerPallet: 256,
        weightLbs: "25.20",
        priceUsd: "43.49",
        casePriceUsd: "43.49",
        palletPriceUsd: "2413.92",
        stockStatus: "in_stock",
        createdAt: new Date(),
      },
      {
        id: 26,
        productId: 7,
        sku: "PP-FRC-1870-15-16B",
        packageSize: "16 BOXES = 64 ROLLS",
        widthInches: "18.00",
        gauge: 70,
        lengthFeet: 1500,
        rollsPerBox: 64,
        rollsPerPallet: 256,
        weightLbs: "403.20",
        priceUsd: "665.06",
        casePriceUsd: "665.06",
        palletPriceUsd: "2413.92",
        stockStatus: "in_stock",
        createdAt: new Date(),
      },
      {
        id: 27,
        productId: 7,
        sku: "PP-FRC-1870-15-32B",
        packageSize: "32 BOXES = 128 ROLLS (HALF PALLET)",
        widthInches: "18.00",
        gauge: 70,
        lengthFeet: 1500,
        rollsPerBox: 128,
        rollsPerPallet: 256,
        weightLbs: "806.40",
        priceUsd: "1268.54",
        casePriceUsd: "1268.54",
        palletPriceUsd: "2413.92",
        stockStatus: "in_stock",
        createdAt: new Date(),
      },
      {
        id: 28,
        productId: 7,
        sku: "PP-FRC-1870-15-64B",
        packageSize: "64 BOXES = 256 ROLLS (FULL PALLET)",
        widthInches: "18.00",
        gauge: 70,
        lengthFeet: 1500,
        rollsPerBox: 256,
        rollsPerPallet: 256,
        weightLbs: "1612.80",
        priceUsd: "2413.92",
        casePriceUsd: "2413.92",
        palletPriceUsd: "2413.92",
        stockStatus: "in_stock",
        createdAt: new Date(),
      },
    ],
  },

  // 8. FORCE STANDARD (80 Gauge Hand Stretch Film - 1500FT)
  {
    id: 8,
    slug: "stretch-film-18-x-80-ga-x-1500ft",
    title: 'STRETCH FILM 18" X 80 GA X 1500FT',
    name: 'STRETCH FILM 18" X 80 GA X 1500FT',
    brand: "FORCE",
    application: "hand",
    categorySlug: "force-standard",
    filmType: "Cast Co-Extruded Multi-Layer",
    color: "Ultra Clear",
    shortDescription:
      "Extended-length 1500FT 80 Gauge maximum-strength industrial cast hand wrap built for extreme weight containment and sharp corner puncture resistance.",
    description:
      "Plastipac FORCE™ Standard 80 Gauge 1500FT Hand Stretch Film delivers maximum containment force and 1500 feet of industrial cast stretch film. Engineered for the heaviest, most demanding loads, steel drums, and abrasive industrial freight.",
    features: [
      "Extended 1500 FT roll length provides maximum wrapping coverage with fewer changeovers",
      "Maximum-strength 80 Gauge cast resin formulation for extreme industrial freight",
      "Extreme puncture durability engineered for jagged pallet edges, metal, and building materials",
      "Optically ultra-clear cast clarity for fast optical barcode and SKU scanning",
      "Whisper-quiet release eliminates warehouse acoustic strain and operator fatigue",
    ],
    techSheetUrl: "/docs/plastipac-force-hand-film-specs.pdf",
    imageUrl:
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/productos_plastipac_manual.png",
    images: [
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/productos_plastipac_manual.png",
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/CAJA_MANUAL_PLASTIPAC.png",
    ],
    recommendedUsage:
      "Manual wrapping of extreme-weight pallets, steel drums, heavy machinery, bagged concrete/cement, and rough export freight.",
    createdAt: new Date(),
    updatedAt: new Date(),
    variants: [
      {
        id: 29,
        productId: 8,
        sku: "PP-FRC-1880-15-1B",
        packageSize: "1 BOX WITH 4 ROLLS",
        widthInches: "18.00",
        gauge: 80,
        lengthFeet: 1500,
        rollsPerBox: 4,
        rollsPerPallet: 256,
        weightLbs: "28.80",
        priceUsd: "49.70",
        casePriceUsd: "49.70",
        palletPriceUsd: "2758.77",
        stockStatus: "in_stock",
        createdAt: new Date(),
      },
      {
        id: 30,
        productId: 8,
        sku: "PP-FRC-1880-15-16B",
        packageSize: "16 BOXES = 64 ROLLS",
        widthInches: "18.00",
        gauge: 80,
        lengthFeet: 1500,
        rollsPerBox: 64,
        rollsPerPallet: 256,
        weightLbs: "460.80",
        priceUsd: "760.07",
        casePriceUsd: "760.07",
        palletPriceUsd: "2758.77",
        stockStatus: "in_stock",
        createdAt: new Date(),
      },
      {
        id: 31,
        productId: 8,
        sku: "PP-FRC-1880-15-32B",
        packageSize: "32 BOXES = 128 ROLLS (HALF PALLET)",
        widthInches: "18.00",
        gauge: 80,
        lengthFeet: 1500,
        rollsPerBox: 128,
        rollsPerPallet: 256,
        weightLbs: "921.60",
        priceUsd: "1449.76",
        casePriceUsd: "1449.76",
        palletPriceUsd: "2758.77",
        stockStatus: "in_stock",
        createdAt: new Date(),
      },
      {
        id: 32,
        productId: 8,
        sku: "PP-FRC-1880-15-64B",
        packageSize: "64 BOXES = 256 ROLLS (FULL PALLET)",
        widthInches: "18.00",
        gauge: 80,
        lengthFeet: 1500,
        rollsPerBox: 256,
        rollsPerPallet: 256,
        weightLbs: "1843.20",
        priceUsd: "2758.77",
        casePriceUsd: "2758.77",
        palletPriceUsd: "2758.77",
        stockStatus: "in_stock",
        createdAt: new Date(),
      },
    ],
  },
  // ==========================================
  // FORCE ELITE (8 Products: 15" x 50, 60, 70, 80 GA for 1000FT & 1500FT)
  // ==========================================
  // 9. FORCE ELITE: 15" X 50 GA X 1000FT
  {
    id: 9,
    slug: "stretch-film-15-x-50-ga-x-1000ft",
    title: 'STRETCH FILM 15" X 50 GA X 1000FT',
    name: 'STRETCH FILM 15" X 50 GA X 1000FT',
    brand: "FORCE ELITE",
    application: "hand",
    categorySlug: "force-elite",
    filmType: "Multi-Layer Nano Technology",
    color: "Ultra Clear",
    shortDescription:
      "Ultra high-yield nano-engineered manual stretch film delivering extreme puncture resistance and high load retention at thinner gauges.",
    description:
      "Plastipac FORCE™ Elite Hand Stretch Film incorporates next-generation nano-layer co-extrusion technology. Designed for demanding manual wrapping applications, it provides maximum containment force, exceptional puncture defense on sharp pallet edges, and reduced wrap weight per unit load for substantial cost savings.",
    features: [
      "Multi-layer nano technology for maximum tensile containment",
      "High optical clarity for fast barcode verification",
      "Differential cling release for smooth, quiet manual unwinding",
      "Superior tear resistance and puncture defense on sharp corners",
      "Optimized yield per roll reducing total plastic consumption",
    ],
    techSheetUrl: "/docs/plastipac-force-hand-film-specs.pdf",
    imageUrl:
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/productos_plastipac_manual.png",
    images: [
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/productos_plastipac_manual.png",
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/CAJA_MANUAL_PLASTIPAC.png",
    ],
    recommendedUsage:
      "High-efficiency manual wrapping of palletized goods, irregular or sharp loads, high-volume shipping hubs, and cold storage distribution.",
    createdAt: new Date(),
    updatedAt: new Date(),
    width_inches: "15.00",
    gauge: 50,
    length_feet: 1000,
    core_type: 'Standard 3" Core',
    variants: [
      {
        id: 33,
        productId: 9,
        sku: "PP-ELT-1550-1B",
        title: "1 BOX WITH 4 ROLLS",
        packageSize: "1 BOX WITH 4 ROLLS",
        widthInches: "15.00",
        gauge: 50,
        lengthFeet: 1000,
        rollsPerBox: 4,
        rollsPerPallet: 256,
        weightLbs: "10.00",
        priceUsd: "17.26",
        casePriceUsd: "17.26",
        palletPriceUsd: "957.91",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 4,
        boxes_count: 1,
      },
      {
        id: 34,
        productId: 9,
        sku: "PP-ELT-1550-16B",
        title: "16 BOXES = 64 ROLLS",
        packageSize: "16 BOXES = 64 ROLLS",
        widthInches: "15.00",
        gauge: 50,
        lengthFeet: 1000,
        rollsPerBox: 64,
        rollsPerPallet: 256,
        weightLbs: "160.00",
        priceUsd: "263.92",
        casePriceUsd: "263.92",
        palletPriceUsd: "957.91",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 64,
        boxes_count: 16,
      },
      {
        id: 35,
        productId: 9,
        sku: "PP-ELT-1550-32B",
        title: "32 BOXES = 128 ROLLS (HALF PALLET)",
        packageSize: "32 BOXES = 128 ROLLS (HALF PALLET)",
        widthInches: "15.00",
        gauge: 50,
        lengthFeet: 1000,
        rollsPerBox: 128,
        rollsPerPallet: 256,
        weightLbs: "320.00",
        priceUsd: "503.39",
        casePriceUsd: "503.39",
        palletPriceUsd: "957.91",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 128,
        boxes_count: 32,
      },
      {
        id: 36,
        productId: 9,
        sku: "PP-ELT-1550-64B",
        title: "64 BOXES = 256 ROLLS (FULL PALLET)",
        packageSize: "64 BOXES = 256 ROLLS (FULL PALLET)",
        widthInches: "15.00",
        gauge: 50,
        lengthFeet: 1000,
        rollsPerBox: 256,
        rollsPerPallet: 256,
        weightLbs: "640.00",
        priceUsd: "957.91",
        casePriceUsd: "957.91",
        palletPriceUsd: "957.91",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 256,
        boxes_count: 64,
      },
    ],
  },
  // 10. FORCE ELITE: 15" X 60 GA X 1000FT
  {
    id: 10,
    slug: "stretch-film-15-x-60-ga-x-1000ft",
    title: 'STRETCH FILM 15" X 60 GA X 1000FT',
    name: 'STRETCH FILM 15" X 60 GA X 1000FT',
    brand: "FORCE ELITE",
    application: "hand",
    categorySlug: "force-elite",
    filmType: "Multi-Layer Nano Technology",
    color: "Ultra Clear",
    shortDescription:
      "Ultra high-yield nano-engineered manual stretch film delivering extreme puncture resistance and high load retention at thinner gauges.",
    description:
      "Plastipac FORCE™ Elite Hand Stretch Film incorporates next-generation nano-layer co-extrusion technology. Designed for demanding manual wrapping applications, it provides maximum containment force, exceptional puncture defense on sharp pallet edges, and reduced wrap weight per unit load for substantial cost savings.",
    features: [
      "Multi-layer nano technology for maximum tensile containment",
      "High optical clarity for fast barcode verification",
      "Differential cling release for smooth, quiet manual unwinding",
      "Superior tear resistance and puncture defense on sharp corners",
      "Optimized yield per roll reducing total plastic consumption",
    ],
    techSheetUrl: "/docs/plastipac-force-hand-film-specs.pdf",
    imageUrl:
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/productos_plastipac_manual.png",
    images: [
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/productos_plastipac_manual.png",
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/CAJA_MANUAL_PLASTIPAC.png",
    ],
    recommendedUsage:
      "High-efficiency manual wrapping of palletized goods, irregular or sharp loads, high-volume shipping hubs, and cold storage distribution.",
    createdAt: new Date(),
    updatedAt: new Date(),
    width_inches: "15.00",
    gauge: 60,
    length_feet: 1000,
    core_type: 'Standard 3" Core',
    variants: [
      {
        id: 37,
        productId: 10,
        sku: "PP-ELT-1560-1B",
        title: "1 BOX WITH 4 ROLLS",
        packageSize: "1 BOX WITH 4 ROLLS",
        widthInches: "15.00",
        gauge: 60,
        lengthFeet: 1000,
        rollsPerBox: 4,
        rollsPerPallet: 256,
        weightLbs: "12.00",
        priceUsd: "20.71",
        casePriceUsd: "20.71",
        palletPriceUsd: "1149.49",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 4,
        boxes_count: 1,
      },
      {
        id: 38,
        productId: 10,
        sku: "PP-ELT-1560-16B",
        title: "16 BOXES = 64 ROLLS",
        packageSize: "16 BOXES = 64 ROLLS",
        widthInches: "15.00",
        gauge: 60,
        lengthFeet: 1000,
        rollsPerBox: 64,
        rollsPerPallet: 256,
        weightLbs: "192.00",
        priceUsd: "316.70",
        casePriceUsd: "316.70",
        palletPriceUsd: "1149.49",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 64,
        boxes_count: 16,
      },
      {
        id: 39,
        productId: 10,
        sku: "PP-ELT-1560-32B",
        title: "32 BOXES = 128 ROLLS (HALF PALLET)",
        packageSize: "32 BOXES = 128 ROLLS (HALF PALLET)",
        widthInches: "15.00",
        gauge: 60,
        lengthFeet: 1000,
        rollsPerBox: 128,
        rollsPerPallet: 256,
        weightLbs: "384.00",
        priceUsd: "604.07",
        casePriceUsd: "604.07",
        palletPriceUsd: "1149.49",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 128,
        boxes_count: 32,
      },
      {
        id: 40,
        productId: 10,
        sku: "PP-ELT-1560-64B",
        title: "64 BOXES = 256 ROLLS (FULL PALLET)",
        packageSize: "64 BOXES = 256 ROLLS (FULL PALLET)",
        widthInches: "15.00",
        gauge: 60,
        lengthFeet: 1000,
        rollsPerBox: 256,
        rollsPerPallet: 256,
        weightLbs: "768.00",
        priceUsd: "1149.49",
        casePriceUsd: "1149.49",
        palletPriceUsd: "1149.49",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 256,
        boxes_count: 64,
      },
    ],
  },
  // 11. FORCE ELITE: 15" X 70 GA X 1000FT
  {
    id: 11,
    slug: "stretch-film-15-x-70-ga-x-1000ft",
    title: 'STRETCH FILM 15" X 70 GA X 1000FT',
    name: 'STRETCH FILM 15" X 70 GA X 1000FT',
    brand: "FORCE ELITE",
    application: "hand",
    categorySlug: "force-elite",
    filmType: "Multi-Layer Nano Technology",
    color: "Ultra Clear",
    shortDescription:
      "Ultra high-yield nano-engineered manual stretch film delivering extreme puncture resistance and high load retention at thinner gauges.",
    description:
      "Plastipac FORCE™ Elite Hand Stretch Film incorporates next-generation nano-layer co-extrusion technology. Designed for demanding manual wrapping applications, it provides maximum containment force, exceptional puncture defense on sharp pallet edges, and reduced wrap weight per unit load for substantial cost savings.",
    features: [
      "Multi-layer nano technology for maximum tensile containment",
      "High optical clarity for fast barcode verification",
      "Differential cling release for smooth, quiet manual unwinding",
      "Superior tear resistance and puncture defense on sharp corners",
      "Optimized yield per roll reducing total plastic consumption",
    ],
    techSheetUrl: "/docs/plastipac-force-hand-film-specs.pdf",
    imageUrl:
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/productos_plastipac_manual.png",
    images: [
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/productos_plastipac_manual.png",
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/CAJA_MANUAL_PLASTIPAC.png",
    ],
    recommendedUsage:
      "High-efficiency manual wrapping of palletized goods, irregular or sharp loads, high-volume shipping hubs, and cold storage distribution.",
    createdAt: new Date(),
    updatedAt: new Date(),
    width_inches: "15.00",
    gauge: 70,
    length_feet: 1000,
    core_type: 'Standard 3" Core',
    variants: [
      {
        id: 41,
        productId: 11,
        sku: "PP-ELT-1570-1B",
        title: "1 BOX WITH 4 ROLLS",
        packageSize: "1 BOX WITH 4 ROLLS",
        widthInches: "15.00",
        gauge: 70,
        lengthFeet: 1000,
        rollsPerBox: 4,
        rollsPerPallet: 256,
        weightLbs: "14.00",
        priceUsd: "24.16",
        casePriceUsd: "24.16",
        palletPriceUsd: "1341.07",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 4,
        boxes_count: 1,
      },
      {
        id: 42,
        productId: 11,
        sku: "PP-ELT-1570-16B",
        title: "16 BOXES = 64 ROLLS",
        packageSize: "16 BOXES = 64 ROLLS",
        widthInches: "15.00",
        gauge: 70,
        lengthFeet: 1000,
        rollsPerBox: 64,
        rollsPerPallet: 256,
        weightLbs: "224.00",
        priceUsd: "369.48",
        casePriceUsd: "369.48",
        palletPriceUsd: "1341.07",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 64,
        boxes_count: 16,
      },
      {
        id: 43,
        productId: 11,
        sku: "PP-ELT-1570-32B",
        title: "32 BOXES = 128 ROLLS (HALF PALLET)",
        packageSize: "32 BOXES = 128 ROLLS (HALF PALLET)",
        widthInches: "15.00",
        gauge: 70,
        lengthFeet: 1000,
        rollsPerBox: 128,
        rollsPerPallet: 256,
        weightLbs: "448.00",
        priceUsd: "704.75",
        casePriceUsd: "704.75",
        palletPriceUsd: "1341.07",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 128,
        boxes_count: 32,
      },
      {
        id: 44,
        productId: 11,
        sku: "PP-ELT-1570-64B",
        title: "64 BOXES = 256 ROLLS (FULL PALLET)",
        packageSize: "64 BOXES = 256 ROLLS (FULL PALLET)",
        widthInches: "15.00",
        gauge: 70,
        lengthFeet: 1000,
        rollsPerBox: 256,
        rollsPerPallet: 256,
        weightLbs: "896.00",
        priceUsd: "1341.07",
        casePriceUsd: "1341.07",
        palletPriceUsd: "1341.07",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 256,
        boxes_count: 64,
      },
    ],
  },
  // 12. FORCE ELITE: 15" X 80 GA X 1000FT
  {
    id: 12,
    slug: "stretch-film-15-x-80-ga-x-1000ft",
    title: 'STRETCH FILM 15" X 80 GA X 1000FT',
    name: 'STRETCH FILM 15" X 80 GA X 1000FT',
    brand: "FORCE ELITE",
    application: "hand",
    categorySlug: "force-elite",
    filmType: "Multi-Layer Nano Technology",
    color: "Ultra Clear",
    shortDescription:
      "Ultra high-yield nano-engineered manual stretch film delivering extreme puncture resistance and high load retention at thinner gauges.",
    description:
      "Plastipac FORCE™ Elite Hand Stretch Film incorporates next-generation nano-layer co-extrusion technology. Designed for demanding manual wrapping applications, it provides maximum containment force, exceptional puncture defense on sharp pallet edges, and reduced wrap weight per unit load for substantial cost savings.",
    features: [
      "Multi-layer nano technology for maximum tensile containment",
      "High optical clarity for fast barcode verification",
      "Differential cling release for smooth, quiet manual unwinding",
      "Superior tear resistance and puncture defense on sharp corners",
      "Optimized yield per roll reducing total plastic consumption",
    ],
    techSheetUrl: "/docs/plastipac-force-hand-film-specs.pdf",
    imageUrl:
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/productos_plastipac_manual.png",
    images: [
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/productos_plastipac_manual.png",
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/CAJA_MANUAL_PLASTIPAC.png",
    ],
    recommendedUsage:
      "High-efficiency manual wrapping of palletized goods, irregular or sharp loads, high-volume shipping hubs, and cold storage distribution.",
    createdAt: new Date(),
    updatedAt: new Date(),
    width_inches: "15.00",
    gauge: 80,
    length_feet: 1000,
    core_type: 'Standard 3" Core',
    variants: [
      {
        id: 45,
        productId: 12,
        sku: "PP-ELT-1580-1B",
        title: "1 BOX WITH 4 ROLLS",
        packageSize: "1 BOX WITH 4 ROLLS",
        widthInches: "15.00",
        gauge: 80,
        lengthFeet: 1000,
        rollsPerBox: 4,
        rollsPerPallet: 256,
        weightLbs: "16.00",
        priceUsd: "27.62",
        casePriceUsd: "27.62",
        palletPriceUsd: "1532.65",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 4,
        boxes_count: 1,
      },
      {
        id: 46,
        productId: 12,
        sku: "PP-ELT-1580-16B",
        title: "16 BOXES = 64 ROLLS",
        packageSize: "16 BOXES = 64 ROLLS",
        widthInches: "15.00",
        gauge: 80,
        lengthFeet: 1000,
        rollsPerBox: 64,
        rollsPerPallet: 256,
        weightLbs: "256.00",
        priceUsd: "422.26",
        casePriceUsd: "422.26",
        palletPriceUsd: "1532.65",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 64,
        boxes_count: 16,
      },
      {
        id: 47,
        productId: 12,
        sku: "PP-ELT-1580-32B",
        title: "32 BOXES = 128 ROLLS (HALF PALLET)",
        packageSize: "32 BOXES = 128 ROLLS (HALF PALLET)",
        widthInches: "15.00",
        gauge: 80,
        lengthFeet: 1000,
        rollsPerBox: 128,
        rollsPerPallet: 256,
        weightLbs: "512.00",
        priceUsd: "805.42",
        casePriceUsd: "805.42",
        palletPriceUsd: "1532.65",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 128,
        boxes_count: 32,
      },
      {
        id: 48,
        productId: 12,
        sku: "PP-ELT-1580-64B",
        title: "64 BOXES = 256 ROLLS (FULL PALLET)",
        packageSize: "64 BOXES = 256 ROLLS (FULL PALLET)",
        widthInches: "15.00",
        gauge: 80,
        lengthFeet: 1000,
        rollsPerBox: 256,
        rollsPerPallet: 256,
        weightLbs: "1024.00",
        priceUsd: "1532.65",
        casePriceUsd: "1532.65",
        palletPriceUsd: "1532.65",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 256,
        boxes_count: 64,
      },
    ],
  },
  // 13. FORCE ELITE: 15" X 50 GA X 1500FT
  {
    id: 13,
    slug: "stretch-film-15-x-50-ga-x-1500ft",
    title: 'STRETCH FILM 15" X 50 GA X 1500FT',
    name: 'STRETCH FILM 15" X 50 GA X 1500FT',
    brand: "FORCE ELITE",
    application: "hand",
    categorySlug: "force-elite",
    filmType: "Multi-Layer Nano Technology",
    color: "Ultra Clear",
    shortDescription:
      "Ultra high-yield nano-engineered manual stretch film delivering extreme puncture resistance and high load retention at thinner gauges.",
    description:
      "Plastipac FORCE™ Elite Hand Stretch Film incorporates next-generation nano-layer co-extrusion technology. Designed for demanding manual wrapping applications, it provides maximum containment force, exceptional puncture defense on sharp pallet edges, and reduced wrap weight per unit load for substantial cost savings.",
    features: [
      "Multi-layer nano technology for maximum tensile containment",
      "High optical clarity for fast barcode verification",
      "Differential cling release for smooth, quiet manual unwinding",
      "Superior tear resistance and puncture defense on sharp corners",
      "Optimized yield per roll reducing total plastic consumption",
    ],
    techSheetUrl: "/docs/plastipac-force-hand-film-specs.pdf",
    imageUrl:
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/productos_plastipac_manual.png",
    images: [
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/productos_plastipac_manual.png",
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/CAJA_MANUAL_PLASTIPAC.png",
    ],
    recommendedUsage:
      "High-efficiency manual wrapping of palletized goods, irregular or sharp loads, high-volume shipping hubs, and cold storage distribution.",
    createdAt: new Date(),
    updatedAt: new Date(),
    width_inches: "15.00",
    gauge: 50,
    length_feet: 1500,
    core_type: 'Standard 3" Core',
    variants: [
      {
        id: 49,
        productId: 13,
        sku: "PP-ELT-1550-15-1B",
        title: "1 BOX WITH 4 ROLLS",
        packageSize: "1 BOX WITH 4 ROLLS",
        widthInches: "15.00",
        gauge: 50,
        lengthFeet: 1500,
        rollsPerBox: 4,
        rollsPerPallet: 256,
        weightLbs: "15.00",
        priceUsd: "25.88",
        casePriceUsd: "25.88",
        palletPriceUsd: "1436.86",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 4,
        boxes_count: 1,
      },
      {
        id: 50,
        productId: 13,
        sku: "PP-ELT-1550-15-16B",
        title: "16 BOXES = 64 ROLLS",
        packageSize: "16 BOXES = 64 ROLLS",
        widthInches: "15.00",
        gauge: 50,
        lengthFeet: 1500,
        rollsPerBox: 64,
        rollsPerPallet: 256,
        weightLbs: "240.00",
        priceUsd: "395.87",
        casePriceUsd: "395.87",
        palletPriceUsd: "1436.86",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 64,
        boxes_count: 16,
      },
      {
        id: 51,
        productId: 13,
        sku: "PP-ELT-1550-15-32B",
        title: "32 BOXES = 128 ROLLS (HALF PALLET)",
        packageSize: "32 BOXES = 128 ROLLS (HALF PALLET)",
        widthInches: "15.00",
        gauge: 50,
        lengthFeet: 1500,
        rollsPerBox: 128,
        rollsPerPallet: 256,
        weightLbs: "480.00",
        priceUsd: "755.08",
        casePriceUsd: "755.08",
        palletPriceUsd: "1436.86",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 128,
        boxes_count: 32,
      },
      {
        id: 52,
        productId: 13,
        sku: "PP-ELT-1550-15-64B",
        title: "64 BOXES = 256 ROLLS (FULL PALLET)",
        packageSize: "64 BOXES = 256 ROLLS (FULL PALLET)",
        widthInches: "15.00",
        gauge: 50,
        lengthFeet: 1500,
        rollsPerBox: 256,
        rollsPerPallet: 256,
        weightLbs: "960.00",
        priceUsd: "1436.86",
        casePriceUsd: "1436.86",
        palletPriceUsd: "1436.86",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 256,
        boxes_count: 64,
      },
    ],
  },
  // 14. FORCE ELITE: 15" X 60 GA X 1500FT
  {
    id: 14,
    slug: "stretch-film-15-x-60-ga-x-1500ft",
    title: 'STRETCH FILM 15" X 60 GA X 1500FT',
    name: 'STRETCH FILM 15" X 60 GA X 1500FT',
    brand: "FORCE ELITE",
    application: "hand",
    categorySlug: "force-elite",
    filmType: "Multi-Layer Nano Technology",
    color: "Ultra Clear",
    shortDescription:
      "Ultra high-yield nano-engineered manual stretch film delivering extreme puncture resistance and high load retention at thinner gauges.",
    description:
      "Plastipac FORCE™ Elite Hand Stretch Film incorporates next-generation nano-layer co-extrusion technology. Designed for demanding manual wrapping applications, it provides maximum containment force, exceptional puncture defense on sharp pallet edges, and reduced wrap weight per unit load for substantial cost savings.",
    features: [
      "Multi-layer nano technology for maximum tensile containment",
      "High optical clarity for fast barcode verification",
      "Differential cling release for smooth, quiet manual unwinding",
      "Superior tear resistance and puncture defense on sharp corners",
      "Optimized yield per roll reducing total plastic consumption",
    ],
    techSheetUrl: "/docs/plastipac-force-hand-film-specs.pdf",
    imageUrl:
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/productos_plastipac_manual.png",
    images: [
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/productos_plastipac_manual.png",
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/CAJA_MANUAL_PLASTIPAC.png",
    ],
    recommendedUsage:
      "High-efficiency manual wrapping of palletized goods, irregular or sharp loads, high-volume shipping hubs, and cold storage distribution.",
    createdAt: new Date(),
    updatedAt: new Date(),
    width_inches: "15.00",
    gauge: 60,
    length_feet: 1500,
    core_type: 'Standard 3" Core',
    variants: [
      {
        id: 53,
        productId: 14,
        sku: "PP-ELT-1560-15-1B",
        title: "1 BOX WITH 4 ROLLS",
        packageSize: "1 BOX WITH 4 ROLLS",
        widthInches: "15.00",
        gauge: 60,
        lengthFeet: 1500,
        rollsPerBox: 4,
        rollsPerPallet: 256,
        weightLbs: "18.00",
        priceUsd: "31.07",
        casePriceUsd: "31.07",
        palletPriceUsd: "1724.78",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 4,
        boxes_count: 1,
      },
      {
        id: 54,
        productId: 14,
        sku: "PP-ELT-1560-15-16B",
        title: "16 BOXES = 64 ROLLS",
        packageSize: "16 BOXES = 64 ROLLS",
        widthInches: "15.00",
        gauge: 60,
        lengthFeet: 1500,
        rollsPerBox: 64,
        rollsPerPallet: 256,
        weightLbs: "288.00",
        priceUsd: "475.19",
        casePriceUsd: "475.19",
        palletPriceUsd: "1724.78",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 64,
        boxes_count: 16,
      },
      {
        id: 55,
        productId: 14,
        sku: "PP-ELT-1560-15-32B",
        title: "32 BOXES = 128 ROLLS (HALF PALLET)",
        packageSize: "32 BOXES = 128 ROLLS (HALF PALLET)",
        widthInches: "15.00",
        gauge: 60,
        lengthFeet: 1500,
        rollsPerBox: 128,
        rollsPerPallet: 256,
        weightLbs: "576.00",
        priceUsd: "906.39",
        casePriceUsd: "906.39",
        palletPriceUsd: "1724.78",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 128,
        boxes_count: 32,
      },
      {
        id: 56,
        productId: 14,
        sku: "PP-ELT-1560-15-64B",
        title: "64 BOXES = 256 ROLLS (FULL PALLET)",
        packageSize: "64 BOXES = 256 ROLLS (FULL PALLET)",
        widthInches: "15.00",
        gauge: 60,
        lengthFeet: 1500,
        rollsPerBox: 256,
        rollsPerPallet: 256,
        weightLbs: "1152.00",
        priceUsd: "1724.78",
        casePriceUsd: "1724.78",
        palletPriceUsd: "1724.78",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 256,
        boxes_count: 64,
      },
    ],
  },
  // 15. FORCE ELITE: 15" X 70 GA X 1500FT
  {
    id: 15,
    slug: "stretch-film-15-x-70-ga-x-1500ft",
    title: 'STRETCH FILM 15" X 70 GA X 1500FT',
    name: 'STRETCH FILM 15" X 70 GA X 1500FT',
    brand: "FORCE ELITE",
    application: "hand",
    categorySlug: "force-elite",
    filmType: "Multi-Layer Nano Technology",
    color: "Ultra Clear",
    shortDescription:
      "Ultra high-yield nano-engineered manual stretch film delivering extreme puncture resistance and high load retention at thinner gauges.",
    description:
      "Plastipac FORCE™ Elite Hand Stretch Film incorporates next-generation nano-layer co-extrusion technology. Designed for demanding manual wrapping applications, it provides maximum containment force, exceptional puncture defense on sharp pallet edges, and reduced wrap weight per unit load for substantial cost savings.",
    features: [
      "Multi-layer nano technology for maximum tensile containment",
      "High optical clarity for fast barcode verification",
      "Differential cling release for smooth, quiet manual unwinding",
      "Superior tear resistance and puncture defense on sharp corners",
      "Optimized yield per roll reducing total plastic consumption",
    ],
    techSheetUrl: "/docs/plastipac-force-hand-film-specs.pdf",
    imageUrl:
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/productos_plastipac_manual.png",
    images: [
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/productos_plastipac_manual.png",
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/CAJA_MANUAL_PLASTIPAC.png",
    ],
    recommendedUsage:
      "High-efficiency manual wrapping of palletized goods, irregular or sharp loads, high-volume shipping hubs, and cold storage distribution.",
    createdAt: new Date(),
    updatedAt: new Date(),
    width_inches: "15.00",
    gauge: 70,
    length_feet: 1500,
    core_type: 'Standard 3" Core',
    variants: [
      {
        id: 57,
        productId: 15,
        sku: "PP-ELT-1570-15-1B",
        title: "1 BOX WITH 4 ROLLS",
        packageSize: "1 BOX WITH 4 ROLLS",
        widthInches: "15.00",
        gauge: 70,
        lengthFeet: 1500,
        rollsPerBox: 4,
        rollsPerPallet: 256,
        weightLbs: "21.00",
        priceUsd: "36.24",
        casePriceUsd: "36.24",
        palletPriceUsd: "2012.68",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 4,
        boxes_count: 1,
      },
      {
        id: 58,
        productId: 15,
        sku: "PP-ELT-1570-15-16B",
        title: "16 BOXES = 64 ROLLS",
        packageSize: "16 BOXES = 64 ROLLS",
        widthInches: "15.00",
        gauge: 70,
        lengthFeet: 1500,
        rollsPerBox: 64,
        rollsPerPallet: 256,
        weightLbs: "336.00",
        priceUsd: "554.47",
        casePriceUsd: "554.47",
        palletPriceUsd: "2012.68",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 64,
        boxes_count: 16,
      },
      {
        id: 59,
        productId: 15,
        sku: "PP-ELT-1570-15-32B",
        title: "32 BOXES = 128 ROLLS (HALF PALLET)",
        packageSize: "32 BOXES = 128 ROLLS (HALF PALLET)",
        widthInches: "15.00",
        gauge: 70,
        lengthFeet: 1500,
        rollsPerBox: 128,
        rollsPerPallet: 256,
        weightLbs: "672.00",
        priceUsd: "1057.69",
        casePriceUsd: "1057.69",
        palletPriceUsd: "2012.68",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 128,
        boxes_count: 32,
      },
      {
        id: 60,
        productId: 15,
        sku: "PP-ELT-1570-15-64B",
        title: "64 BOXES = 256 ROLLS (FULL PALLET)",
        packageSize: "64 BOXES = 256 ROLLS (FULL PALLET)",
        widthInches: "15.00",
        gauge: 70,
        lengthFeet: 1500,
        rollsPerBox: 256,
        rollsPerPallet: 256,
        weightLbs: "1344.00",
        priceUsd: "2012.68",
        casePriceUsd: "2012.68",
        palletPriceUsd: "2012.68",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 256,
        boxes_count: 64,
      },
    ],
  },
  // 16. FORCE ELITE: 15" X 80 GA X 1500FT
  {
    id: 16,
    slug: "stretch-film-15-x-80-ga-x-1500ft",
    title: 'STRETCH FILM 15" X 80 GA X 1500FT',
    name: 'STRETCH FILM 15" X 80 GA X 1500FT',
    brand: "FORCE ELITE",
    application: "hand",
    categorySlug: "force-elite",
    filmType: "Multi-Layer Nano Technology",
    color: "Ultra Clear",
    shortDescription:
      "Ultra high-yield nano-engineered manual stretch film delivering extreme puncture resistance and high load retention at thinner gauges.",
    description:
      "Plastipac FORCE™ Elite Hand Stretch Film incorporates next-generation nano-layer co-extrusion technology. Designed for demanding manual wrapping applications, it provides maximum containment force, exceptional puncture defense on sharp pallet edges, and reduced wrap weight per unit load for substantial cost savings.",
    features: [
      "Multi-layer nano technology for maximum tensile containment",
      "High optical clarity for fast barcode verification",
      "Differential cling release for smooth, quiet manual unwinding",
      "Superior tear resistance and puncture defense on sharp corners",
      "Optimized yield per roll reducing total plastic consumption",
    ],
    techSheetUrl: "/docs/plastipac-force-hand-film-specs.pdf",
    imageUrl:
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/productos_plastipac_manual.png",
    images: [
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/productos_plastipac_manual.png",
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/CAJA_MANUAL_PLASTIPAC.png",
    ],
    recommendedUsage:
      "High-efficiency manual wrapping of palletized goods, irregular or sharp loads, high-volume shipping hubs, and cold storage distribution.",
    createdAt: new Date(),
    updatedAt: new Date(),
    width_inches: "15.00",
    gauge: 80,
    length_feet: 1500,
    core_type: 'Standard 3" Core',
    variants: [
      {
        id: 61,
        productId: 16,
        sku: "PP-ELT-1580-15-1B",
        title: "1 BOX WITH 4 ROLLS",
        packageSize: "1 BOX WITH 4 ROLLS",
        widthInches: "15.00",
        gauge: 80,
        lengthFeet: 1500,
        rollsPerBox: 4,
        rollsPerPallet: 256,
        weightLbs: "24.00",
        priceUsd: "41.42",
        casePriceUsd: "41.42",
        palletPriceUsd: "2300.58",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 4,
        boxes_count: 1,
      },
      {
        id: 62,
        productId: 16,
        sku: "PP-ELT-1580-15-16B",
        title: "16 BOXES = 64 ROLLS",
        packageSize: "16 BOXES = 64 ROLLS",
        widthInches: "15.00",
        gauge: 80,
        lengthFeet: 1500,
        rollsPerBox: 64,
        rollsPerPallet: 256,
        weightLbs: "384.00",
        priceUsd: "633.73",
        casePriceUsd: "633.73",
        palletPriceUsd: "2300.58",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 64,
        boxes_count: 16,
      },
      {
        id: 63,
        productId: 16,
        sku: "PP-ELT-1580-15-32B",
        title: "32 BOXES = 128 ROLLS (HALF PALLET)",
        packageSize: "32 BOXES = 128 ROLLS (HALF PALLET)",
        widthInches: "15.00",
        gauge: 80,
        lengthFeet: 1500,
        rollsPerBox: 128,
        rollsPerPallet: 256,
        weightLbs: "768.00",
        priceUsd: "1208.99",
        casePriceUsd: "1208.99",
        palletPriceUsd: "2300.58",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 128,
        boxes_count: 32,
      },
      {
        id: 64,
        productId: 16,
        sku: "PP-ELT-1580-15-64B",
        title: "64 BOXES = 256 ROLLS (FULL PALLET)",
        packageSize: "64 BOXES = 256 ROLLS (FULL PALLET)",
        widthInches: "15.00",
        gauge: 80,
        lengthFeet: 1500,
        rollsPerBox: 256,
        rollsPerPallet: 256,
        weightLbs: "1536.00",
        priceUsd: "2300.58",
        casePriceUsd: "2300.58",
        palletPriceUsd: "2300.58",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 256,
        boxes_count: 64,
      },
    ],
  },
  // ==========================================
  // GENESIS STANDARD (4 Machine Products: 20" x 60, 70, 80 GA for 5000FT & 6000FT)
  // ==========================================
  // 17. GENESIS STANDARD: 20" X 60 GA X 5000FT
  {
    id: 17,
    slug: "stretch-film-20-x-60-ga-x-5000ft",
    title: 'STRETCH FILM 20" X 60 GA X 5000FT',
    name: 'STRETCH FILM 20" X 60 GA X 5000FT',
    brand: "GENESIS",
    application: "machine",
    categorySlug: "genesis-standard",
    categoryId: "b0000000-0000-0000-0000-000000000003",
    filmType: "Cast Machine Stretch Film",
    color: "Ultra Clear",
    shortDescription:
      "High-yield automated cast stretch film engineered for high-speed turntable and rotary wrapper systems.",
    description:
      "Plastipac GENESIS™ Machine Stretch Film is designed for automated warehouse wrapping applications. Formulated with premium metallocene resins, it delivers high pre-stretch levels, exceptional puncture resistance on sharp pallet corners, and rock-solid load stability during long-haul transport.",
    features: [
      "Ultra-high pre-stretch capability up to 300% for maximum film economy",
      "Superior load retention force prevents pallet shifting in transit",
      "High puncture defense for irregular or pallet overhang profiles",
      "Quiet high-speed unwinding on automatic and semi-automatic turntables",
      "Exceptional optical clarity for high-speed automated barcode scanning",
    ],
    techSheetUrl: "/docs/plastipac-force-hand-film-specs.pdf",
    imageUrl:
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/AUTOMATIC_STRETCH_FILM.png",
    images: [
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/AUTOMATIC_STRETCH_FILM.png",
    ],
    recommendedUsage:
      "Automated stretch wrapping lines, high-throughput logistics hubs, distribution centers, and standard machine pallet wrapping.",
    createdAt: new Date(),
    updatedAt: new Date(),
    width_inches: "20.00",
    gauge: 60,
    length_feet: 5000,
    core_type: 'Standard 3" Core',
    variants: [
      {
        id: 65,
        productId: 17,
        sku: "PP-GEN-2060-1R",
        title: "1 MACHINE ROLL",
        packageSize: "1 MACHINE ROLL",
        widthInches: "20.00",
        gauge: 60,
        lengthFeet: 5000,
        rollsPerBox: 1,
        rollsPerPallet: 40,
        weightLbs: "33.60",
        priceUsd: "192.44",
        casePriceUsd: "192.44",
        palletPriceUsd: "7697.60",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 1,
        boxes_count: 1,
      },
      {
        id: 66,
        productId: 17,
        sku: "PP-GEN-2060-40R",
        title: "40 ROLLS (FULL PALLET)",
        packageSize: "40 ROLLS (FULL PALLET)",
        widthInches: "20.00",
        gauge: 60,
        lengthFeet: 5000,
        rollsPerBox: 40,
        rollsPerPallet: 40,
        weightLbs: "1344.00",
        priceUsd: "7697.60",
        casePriceUsd: "7697.60",
        palletPriceUsd: "7697.60",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 40,
        boxes_count: 40,
      },
    ],
  },
  // 18. GENESIS STANDARD: 20" X 70 GA X 6000FT
  {
    id: 18,
    slug: "stretch-film-20-x-70-ga-x-6000ft",
    title: 'STRETCH FILM 20" X 70 GA X 6000FT',
    name: 'STRETCH FILM 20" X 70 GA X 6000FT',
    brand: "GENESIS",
    application: "machine",
    categorySlug: "genesis-standard",
    categoryId: "b0000000-0000-0000-0000-000000000003",
    filmType: "Cast Machine Stretch Film",
    color: "Ultra Clear",
    shortDescription:
      "High-yield automated cast stretch film engineered for high-speed turntable and rotary wrapper systems.",
    description:
      "Plastipac GENESIS™ Machine Stretch Film is designed for automated warehouse wrapping applications. Formulated with premium metallocene resins, it delivers high pre-stretch levels, exceptional puncture resistance on sharp pallet corners, and rock-solid load stability during long-haul transport.",
    features: [
      "Ultra-high pre-stretch capability up to 300% for maximum film economy",
      "Superior load retention force prevents pallet shifting in transit",
      "High puncture defense for irregular or pallet overhang profiles",
      "Quiet high-speed unwinding on automatic and semi-automatic turntables",
      "Exceptional optical clarity for high-speed automated barcode scanning",
    ],
    techSheetUrl: "/docs/plastipac-force-hand-film-specs.pdf",
    imageUrl:
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/AUTOMATIC_STRETCH_FILM.png",
    images: [
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/AUTOMATIC_STRETCH_FILM.png",
    ],
    recommendedUsage:
      "Automated stretch wrapping lines, high-throughput logistics hubs, distribution centers, and standard machine pallet wrapping.",
    createdAt: new Date(),
    updatedAt: new Date(),
    width_inches: "20.00",
    gauge: 70,
    length_feet: 6000,
    core_type: 'Standard 3" Core',
    variants: [
      {
        id: 67,
        productId: 18,
        sku: "PP-GEN-2070-1R",
        title: "1 MACHINE ROLL",
        packageSize: "1 MACHINE ROLL",
        widthInches: "20.00",
        gauge: 70,
        lengthFeet: 6000,
        rollsPerBox: 1,
        rollsPerPallet: 40,
        weightLbs: "47.04",
        priceUsd: "224.51",
        casePriceUsd: "224.51",
        palletPriceUsd: "8980.40",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 1,
        boxes_count: 1,
      },
      {
        id: 68,
        productId: 18,
        sku: "PP-GEN-2070-40R",
        title: "40 ROLLS (FULL PALLET)",
        packageSize: "40 ROLLS (FULL PALLET)",
        widthInches: "20.00",
        gauge: 70,
        lengthFeet: 6000,
        rollsPerBox: 40,
        rollsPerPallet: 40,
        weightLbs: "1881.60",
        priceUsd: "8980.40",
        casePriceUsd: "8980.40",
        palletPriceUsd: "8980.40",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 40,
        boxes_count: 40,
      },
    ],
  },
  // 19. GENESIS STANDARD: 20" X 60 GA X 6000FT
  {
    id: 19,
    slug: "stretch-film-20-x-60-ga-x-6000ft-1",
    title: 'STRETCH FILM 20" X 60 GA X 6000FT',
    name: 'STRETCH FILM 20" X 60 GA X 6000FT',
    brand: "GENESIS",
    application: "machine",
    categorySlug: "genesis-standard",
    categoryId: "b0000000-0000-0000-0000-000000000003",
    filmType: "Cast Machine Stretch Film",
    color: "Ultra Clear",
    shortDescription:
      "High-yield automated cast stretch film engineered for high-speed turntable and rotary wrapper systems.",
    description:
      "Plastipac GENESIS™ Machine Stretch Film is designed for automated warehouse wrapping applications. Formulated with premium metallocene resins, it delivers high pre-stretch levels, exceptional puncture resistance on sharp pallet corners, and rock-solid load stability during long-haul transport.",
    features: [
      "Ultra-high pre-stretch capability up to 300% for maximum film economy",
      "Superior load retention force prevents pallet shifting in transit",
      "High puncture defense for irregular or pallet overhang profiles",
      "Quiet high-speed unwinding on automatic and semi-automatic turntables",
      "Exceptional optical clarity for high-speed automated barcode scanning",
    ],
    techSheetUrl: "/docs/plastipac-force-hand-film-specs.pdf",
    imageUrl:
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/AUTOMATIC_STRETCH_FILM.png",
    images: [
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/AUTOMATIC_STRETCH_FILM.png",
    ],
    recommendedUsage:
      "Automated stretch wrapping lines, high-throughput logistics hubs, distribution centers, and standard machine pallet wrapping.",
    createdAt: new Date(),
    updatedAt: new Date(),
    width_inches: "20.00",
    gauge: 60,
    length_feet: 6000,
    core_type: 'Standard 3" Core',
    variants: [
      {
        id: 69,
        productId: 19,
        sku: "PP-GEN-2060-60-1R",
        title: "1 MACHINE ROLL",
        packageSize: "1 MACHINE ROLL",
        widthInches: "20.00",
        gauge: 60,
        lengthFeet: 6000,
        rollsPerBox: 1,
        rollsPerPallet: 40,
        weightLbs: "40.32",
        priceUsd: "205.28",
        casePriceUsd: "205.28",
        palletPriceUsd: "8211.20",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 1,
        boxes_count: 1,
      },
      {
        id: 70,
        productId: 19,
        sku: "PP-GEN-2060-60-40R",
        title: "40 ROLLS (FULL PALLET)",
        packageSize: "40 ROLLS (FULL PALLET)",
        widthInches: "20.00",
        gauge: 60,
        lengthFeet: 6000,
        rollsPerBox: 40,
        rollsPerPallet: 40,
        weightLbs: "1612.80",
        priceUsd: "8211.20",
        casePriceUsd: "8211.20",
        palletPriceUsd: "8211.20",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 40,
        boxes_count: 40,
      },
    ],
  },
  // 20. GENESIS STANDARD: 20" X 80 GA X 5000FT
  {
    id: 20,
    slug: "stretch-film-20-x-80-ga-x-5000ft-1",
    title: 'STRETCH FILM 20" X 80 GA X 5000FT',
    name: 'STRETCH FILM 20" X 80 GA X 5000FT',
    brand: "GENESIS",
    application: "machine",
    categorySlug: "genesis-standard",
    categoryId: "b0000000-0000-0000-0000-000000000003",
    filmType: "Cast Machine Stretch Film",
    color: "Ultra Clear",
    shortDescription:
      "High-yield automated cast stretch film engineered for high-speed turntable and rotary wrapper systems.",
    description:
      "Plastipac GENESIS™ Machine Stretch Film is designed for automated warehouse wrapping applications. Formulated with premium metallocene resins, it delivers high pre-stretch levels, exceptional puncture resistance on sharp pallet corners, and rock-solid load stability during long-haul transport.",
    features: [
      "Ultra-high pre-stretch capability up to 300% for maximum film economy",
      "Superior load retention force prevents pallet shifting in transit",
      "High puncture defense for irregular or pallet overhang profiles",
      "Quiet high-speed unwinding on automatic and semi-automatic turntables",
      "Exceptional optical clarity for high-speed automated barcode scanning",
    ],
    techSheetUrl: "/docs/plastipac-force-hand-film-specs.pdf",
    imageUrl:
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/AUTOMATIC_STRETCH_FILM.png",
    images: [
      "https://ahvmjptomjjnqjylofpa.supabase.co/storage/v1/object/public/Products/AUTOMATIC_STRETCH_FILM.png",
    ],
    recommendedUsage:
      "Automated stretch wrapping lines, high-throughput logistics hubs, distribution centers, and standard machine pallet wrapping.",
    createdAt: new Date(),
    updatedAt: new Date(),
    width_inches: "20.00",
    gauge: 80,
    length_feet: 5000,
    core_type: 'Standard 3" Core',
    variants: [
      {
        id: 71,
        productId: 20,
        sku: "PP-GEN-2080-1R",
        title: "1 MACHINE ROLL",
        packageSize: "1 MACHINE ROLL",
        widthInches: "20.00",
        gauge: 80,
        lengthFeet: 5000,
        rollsPerBox: 1,
        rollsPerPallet: 40,
        weightLbs: "44.80",
        priceUsd: "256.59",
        casePriceUsd: "256.59",
        palletPriceUsd: "10263.60",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 1,
        boxes_count: 1,
      },
      {
        id: 72,
        productId: 20,
        sku: "PP-GEN-2080-40R",
        title: "40 ROLLS (FULL PALLET)",
        packageSize: "40 ROLLS (FULL PALLET)",
        widthInches: "20.00",
        gauge: 80,
        lengthFeet: 5000,
        rollsPerBox: 40,
        rollsPerPallet: 40,
        weightLbs: "1792.00",
        priceUsd: "10263.60",
        casePriceUsd: "10263.60",
        palletPriceUsd: "10263.60",
        stockStatus: "in_stock",
        createdAt: new Date(),
        rolls_count: 40,
        boxes_count: 40,
      },
    ],
  },
];
